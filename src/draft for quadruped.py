...
def smooth_move_servo(servo, start_duty, end_duty, steps=10):
    step_size = (end_duty - start_duty) / steps
    for step in range(steps):
        duty = int(start_duty + step * step_size)
        servo.duty(duty)
        sleep(crawl_speed)
    ...
    for leg_group in tripod_legs:
        for leg in leg_group:
            ...
            if angles:
                ...
                smooth_move_servo(servos[servo1], servos[servo1].duty(), duty1)
                smooth_move_servo(servos[servo2], servos[servo2].duty(), (theta2))
